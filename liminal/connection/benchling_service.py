...
class BenchlingService(Benchling):
    """
    Class that creates a connection object for connecting to Benchling's API, database, or internal API.

    Parameters
    ----------
    connection: BenchlingConnection
        The connection object that contains the credentials and registry ID for the Benchling tenant.
    use_api: bool
    ...
    def __init__(
        ...
        self._registry_id = connection.registry_id  # Use registry_id from connection
    ...
    def registry_id(self) -> str:
        """
        Retrieves the registry ID for the tenant. If a registry_id was provided in the
        BenchlingConnection, it will be used. If not, and there are multiple registries,
        a ValueError will be raised.

        Returns
        -------
        str
            The registry ID of the selected or default registry.
        """
        # Use the registry_id specified in the connection if available
        if self._registry_id:
            registries = self.registry.registries()
            for registry in registries:
                if registry.id == self._registry_id:
                    return registry.id
            raise ValueError(f"Registry with id '{self._registry_id}' not found.")
        else:
            # Default behavior if no registry_id is provided
            registries = self.registry.registries()
            if len(registries) == 1:
                return registries[0].id
            raise ValueError("Multiple registries found. Please specify a registry_id.")
